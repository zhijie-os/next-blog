    ...
    vector<int> findBall(vector<vector<int>>& grid) {
        ...
        for(int i=0; i<grid[0].size(); i++){
            ...
            while(row<grid.size()){
                ...
                else{ // otherwise <-> |x/, \x|, \x/
                    break;
                }
            }
            
            result.push_back(row==grid.size()?col:-1);
        }
        return result;
    }
};
...
- Difficulty: Medium
- Link: [328. Odd Even Linked List](https://leetcode.com/problems/odd-even-linked-list)
...
public:
    ListNode* oddEvenList(ListNode* head) {
        if(head){
            ...
            // the major question is when to stop
            // odd number of nodes => only 1 node left
            // even number of nodes => when there are two node left
            while(even && even->next) {
                // remove the next connection for the even element
                odd->next = odd->next->next;
                odd = odd->next;
                // remove the next connection for the odd element
